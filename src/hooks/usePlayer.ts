...
import { makeL1Connection, L1_RPC, ANCHOR_PROGRAM_ID, STEEL_PROGRAM_ID, POOL_PDA, TREASURY, PLAYER_CLAIMABLE_SOL_OFFSET } from '@/lib/constants';
...
export function usePlayer(): UsePlayerReturn {
  const { publicKey, sendTransaction, isConnected: connected } = useUnifiedWallet();
  ...
  const register = useCallback(async (): Promise<string> => {
    ...
    // Pre-flight balance check (shared util — see lib/assertFunds.ts).
    await assertFunds({
      connection,
      payer: publicKey,
      requiredLamports: FUNDS_HINTS.REGISTER_LAMPORTS,
      reason: 'Registering creates a player account on-chain — this needs SOL for rent.',
    });

    // Get recent blockhash
    const { blockhash } = await getLatestBlockhashClient(connection, 'confirmed');
